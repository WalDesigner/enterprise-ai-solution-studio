...
> 文档状态：当前有效（2026-09-10，Asia/Shanghai；本轮未重新触发线上 Real AI）
>
...
| 已核验发布源码 | `51056d2`（PR #6）；不以变化中的 main 代替发布提交 |
...
| 当前已核验版本 | `011`，控制面正常，100% 流量；`010` 为 0% |
...
2026-09-10：CLI 发布 011 后，通过部署记录只读回读确认版本正常、流量 100%。
本轮继承已有环境变量与实例策略，没有重新调用真实 AI。后续整站前端改动需要另行发布，
不能把 011 的成功套用到尚未发布的源码。
...
- `011`：2026-09-10 发布，来源 `51056d2`；正常、100% 流量；本轮没有 Real AI 复测。
