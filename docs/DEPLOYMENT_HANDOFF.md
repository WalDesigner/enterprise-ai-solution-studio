...
> 文档状态：当前有效（2026-09-11，Asia/Shanghai；本轮未重新触发线上 Real AI）
>
...
| 已核验发布源码 | `9e22a9751daffc9dc2254a7fb4bef88bc777b96b`（PR #7）；后续文档提交不改变运行源码 |
...
| 当前已核验版本 | `012`，控制面正常，100% 流量；`011` 为 0% |
...
2026-09-11：整站流程页优化发布为 012，构建与发布步骤均已完成；部署记录为 normal / 100%。本轮未回滚。
继承已有环境变量与实例策略，没有调用真实 AI，没有购买、启用收费项或修改域名。

本轮公网证据：
- 七条路由 `/`、`/customers`、`/analysis`、`/solution`、`/poc`、`/deployment`、`/roi` HTTP 200。
- 注意：单独 HTTP 200 可能是平台提示页，不能据此断言应用可用。
- 实际浏览器确认进入上述七页并核对新版标题；需求输入可见。
- 实际选择“审批流程协同”后进入 PoC → 部署 → ROI，仍对应北方海工客户：
  投入 ¥140,000、年度节省估算 ¥226,000、ROI 61%、回收期 7.4 个月。
- 首次访问遇到 CloudBase“测试域名”中间提示，点击其“确定访问”后正常进入。
  这是当前免费默认域名的访问摩擦，未消除；不能在交接中称为无提示直达。
- 本轮 AI API 未测；保留下面带日期的历史 Real AI 证据。

历史证据：截至 2026-08-20，CloudBase `010` 的首页和 `/analysis` 已再次返回 HTTP 200；最近
...
每次发布至少记录：发布日期、代码提交、CloudBase 版本、流量结果、页面状态、浏览器结果、是否回滚。只记录证据，不记录密钥或敏感响应正文。

本轮前端维护例外：本人明确要求不重复调用真实 AI。后端、Provider 和环境变量未改时，
执行离线单元测试、模拟响应的浏览器回归和公网页面/客户切换验证即可；
API 三个诊断字段标为“本轮未测，沿用有日期的历史记录”，不能伪写为重新验收通过。
若后端、Provider 配置或模型发生变化，重新取得真实调用授权后再执行 7.2 / 7.3。
...
- `012`：2026-09-11 发布，来源 `9e22a97`；正常、100% 流量；页面和客户切换实测通过，未调用真实 AI。
- `011`：2026-09-10 发布，来源 `51056d2`；当前 0% 流量，作为本轮前的回滚源码参考。
- `010`：旧版本，曾完成 2026-08-19 公网 Real AI 验收；可作为历史回滚参考。
